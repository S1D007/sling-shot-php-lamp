...
<!--</div>-->
  ...
  <script>
    ...
    var slingShotId = document.getElementById('slingShotId').value;
    const socket = io('https://api.gokapturehub.com', {
      transports: ['websocket'],
    });
    var orientation;
    socket.on("connect", () => {
      socket.on(<?php echo $_GET['id'];?>,function (data) {
        // var new_d = JSON.stringify(data);
          // const orientation = data;      
                orientation = data;
                // orientation_data = data;
            if( (orientation.gama > 0 || orientation.gama < 0) &&  (orientation.beta > 0 || orientation.beta < 0)){
                            
              updateTrackerPosition(orientation.gama, orientation.beta);
              if(gamma_old!= orientation.gama && beta_old != orientation.beta){
                gamma_old= orientation.gama;
...
function fetchNotes() {
  $.ajax({
    url: 'fetch_notes.php?id='+slingShotId,
    dataType: 'json',
    success: function(data) {
      // Store the fetched notes
      notes = data.notes;
      $.ajax({
        url: 'row_count.php?id='+slingShotId,
        success: function(data) {
          // Store the fetched notes
      ...
      try {
        ...
        notes.forEach(function(note, index) {
          ...
          const imageUrl = imageFolder + randomIndex + '.png';
          ...
          while (overlap) {
            ...
            for (var i = 0; i < notes.length; i++) {
              if (i !== index) {
                var otherNote = notes[i];
                var distanceX = Math.abs(randomLeft - otherNote.left);
                var distanceY = Math.abs(randomTop - otherNote.top);
          ...
          else if(note.img_id == 3){
            // margin_left = '15px';
             margin_top = '-100px';
             note_message_bottom = '133px';
          }
          ...
          // }
          else if(note.img_id == 2){
                 // margin_left = '15px';
                 margin_top = '-100px';
                 note_message_bottom = '133px';
          }
          else if(note.img_id == 1){
                // margin_left = '15px';
                 margin_top = '-100px';
                 note_message_bottom = '133px';
          }
          else if(note.img_id == 5){
            //   margin_top = '55px';
            margin_top = '-85px';
            note_message_bottom = '115px';
          }
          else if(note.img_id == 6){
            margin_top ='-100px';
            note_message_bottom = '130px';        
          }
          else{
          ...
          var final_message =''
          if(note.message.toString() == ''){
            
          }
          else{
            final_message = '<div class="message">'+note.message+'</div>';
          }

          var noteHTML = `<div class="note" style="background:transparent;top:` + randomTop + `px;left:` + randomLeft + `px;z-index:` + note.id + `;width:`+ width +`;">` +
          ...
          //   `<div style="width: 100%;height: 70%;position: absolute;"><img src="./assets/img/` + note.img_id + `.png" style="width: 100%;height: 100%;background-color: transparent;"></div><div class="note-image"></div>` +
          //   '<div class="note-header" style="margin-top:'+ margin_top +';margin-left:'+ margin_left +'"><img src="' + note.photo + '">' +
          //   '<div class="note-message" style="bottom:125px;"><p>' + note.name + '</p></div></div>' +
          //   '' +
          //   '</div>';
          // }
          // else if(note.img_id == 5){
              
          //     var noteHTML = `<div class="note" style="background:transparent;top:` + randomTop + `px;left:` + randomLeft + `px;z-index:` + note.id + `;">` +
          //   `<div style="width: 100%;height: 70%;position: absolute;"><img src="./assets/img/` + note.img_id + `.png" style="width: 100%;height: 100%;background-color: transparent;"></div><div class="note-image"></div>` +
          //   '<div class="note-header" style="margin-top:'+ margin_top +';margin-left:'+ margin_left +'"><img src="' + note.photo + '">' +
          //   '<div class="note-message" style="bottom:135px;"><p>' + note.name + '</p></div></div>' +
          //   '' +
          //   '</div>';
          // }
          // else if(note.img_id == 3){
          ...
          //   `<div style="width: 100%;height: 70%;position: absolute;"><img src="./assets/img/` + note.img_id + `.png" style="width: 100%;height: 100%;background-color: transparent;"></div><div class="note-image"></div>` +
          //   '<div class="note-header" style="margin-top:'+ margin_top +';margin-left:'+ margin_left +'"><img src="' + note.photo + '">' +
          //   '<div class="note-message" style="top:115px;"><p>' + note.name + '</p></div></div>' +
          //   '' +
          //   '</div>';
          // }
          // else if(note.img_id == 1){
          ...
          //   `<div style="width: 100%;height: 70%;position: absolute;"><img src="./assets/img/` + note.img_id + `.png" style="width: 100%;height: 100%;background-color: transparent;"></div><div class="note-image"></div>` +
          //   '<div class="note-header" style="margin-top:'+ margin_top +';margin-left:'+ margin_left +'"><img src="' + note.photo + '">' +
          //   '<div class="note-message" style="bottom:115px;"><p>' + note.name + '</p></div></div>' +
          //   '' +
          //   '</div>';
          // }
          // else if(note.img_id == 2){
          ...
          //   `<div style="width: 100%;height: 70%;position: absolute;"><img src="./assets/img/` + note.img_id + `.png" style="width: 100%;height: 100%;background-color: transparent;"></div><div class="note-image"></div>` +
          //   '<div class="note-header" style="margin-top:'+ margin_top +';margin-left:'+ margin_left +'"><img src="' + note.photo + '">' +
          //   '<div class="note-message"><p>' + note.name + '</p></div></div>' +
          //   '' +
          //   '</div>';
          // }
  ...
  await $.ajax({
    url: 'fetch_latest_row.php?id='+slingShotId,
    success: function(note) {
      ...
      var get_current_pos = $('#tracker').offset();
        ...
        if(note[i].message.toString() == ''){
          ...
          var noteHTML = `<div class="note" style="background:transparent;top:` + (get_current_pos.top-40) + `px;left:` + (get_current_pos.left-100) + `px;z-index:` + note[i].ID + `;width:`+ width +`;">` +
      ...
      var noteHTML2 = `<div class="note" id="note-`+note[i].ID+`" style="background:transparent;top:` + randomTop2 + `px;left:` + randomLeft2 + `px;z-index:` + note[i].ID + `;width:400px;height:400px;">` +
            `<div style="width: 100%;height: 70%;position: absolute;"><img src="./assets/img/` + note[i].img_id + `.png" style="width: 100%;height: 100%;background-color: transparent;"></div><div class="note-image"></div>` +
            '<div class="note-header" style="margin-top:'+ margin_top +';margin-left:'+ margin_left +'"><img src="' + note[i].photo + '">' +
            '<div class="note-message" style="bottom:'+note_message_bottom+'"><p style="position:relative;">' + note[i].name + '</p></div>'+final_message+'</div>' +
            '</div>';  
